...
class WhitespaceTokenizeWithOffsetsV2Op
 ...
 public:
  ...
  static constexpr char kOpName[] = "WhitespaceTokenizeWithOffsetsV2";
  ...
  Returns:
    ...
    * output_row_splits: 1D int tensor with the row splits that allow us to
      ...
      output_end_offsets.
    * output_start_offsets: 1D tensor containing the inclusive start byte offset for
...
#define FILL_OUTPUT_TENSOR(name, index, dtype)                                \
  SH_ASSIGN_OR_RETURN(const auto name##_tensorview, context->GetOutput(index, \
      tflite::shim::Shape({(int) name.size()})));                             \
  auto name##_data = name##_tensorview->template As<dtype, 1>();              \
